import { getCustomFormulas, getOptions, showUsage } from "main/lib";
...
export async function main(ns) {
	...
	let host =
		options.host && options.host.length > 0 ? options.host : ns.getHostname();
	...
	for (let i = 0; i < targetList.length; i++) {
		...
		while (
			...
			let availableThreads = Math.trunc(
				availableRam /
					Math.max(ns.getScriptRam(weakenScript), ns.getScriptRam(growScript))
			);

			if (availableThreads === 0) {
				ns.print(
					`WARNING: No threads available on ${host}, sleeping 10 seconds`
				);
			...
			let requiredGrowThreads = Math.ceil(
				(server.moneyMax - server.moneyAvailable) /
					(server.moneyAvailable || player.skills.hacking + 1) /
					growPercent
			);
			let requiredWeakenThreads = Math.ceil(
				(server.hackDifficulty - server.minDifficulty) / 0.05
			);
			ns.print(
				`\x1b[38;5;13m${server.hostname}: \x1b[38;5;219mGrows needed: ${requiredGrowThreads}, Weakens needed: ${requiredWeakenThreads}, Available: ${availableThreads}`
			);
			const now = new Date(new Date().valueOf()).toLocaleTimeString();
			const finish = new Date(
				new Date().valueOf() + weakenTime + 500
			).toLocaleTimeString();
			if (requiredWeakenThreads > availableThreads) {
				ns.exec(weakenScript, host, availableThreads, target, now, finish);
				ns.print(
					`weakening only for ${ns.nFormat(weakenTime / 1000, "0.0")} seconds`
				);
				...
				ns.print(
					`weakening x ${weakenThreads} in ${ns.nFormat(
						weakenTime / 1000,
						"0.0"
					)} seconds`
				);
				...
				if (
					...
					growThreads < requiredGrowThreads
				) {
					await ns.sleep(500 + weakenTime);
				} else {
