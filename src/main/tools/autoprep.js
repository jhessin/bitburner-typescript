...
export async function main(ns) {
	...
	if (options.targets && options.targets.length > 0) {
		...
		targetList = targetList.filter(
			(x) => x.hackDifficulty > x.minDifficulty || x.moneyAvailable < x.moneyMax
		);
	...
	if (reserve) {
		ns.tprint(`Reserving ${reserve} on ${host}`);
	}
	...
	while (true) {
		...
		Object.entries(runningScripts).forEach((x) => {
			if (x[1]) {
				...
				if (!(x[1].weak2 || x[1].weak || x[1].grow) && !continuePrepping)
					delete runningScripts[hostname];
			}
		});
		...
		for (let i = 0; i < targets.length; i++) {
			...
			if (!(weak || grow || weak2)) {
				...
				if (wt) {
					...
					ns.print(
						`${finish} - weak ${target.hostname}: ${wt} in ${ns.nFormat(
							weakenTime / 1000,
							"0,000"
						)} seconds`
					);
					...
					runningScripts[target.hostname] = Object.assign(
						{},
						runningScripts[target.hostname],
						{ weak: pid }
					);
				}
			}
		...
		for (let i = 0; i < targets.length; i++) {
			...
			if (!(grow || weak2) && (!weak || addedWeak[target.hostname])) {
				...
				let growPercent = hacking.growPercent(
					target,
					1,
					player,
					hostServer.cpuCores
				...
				let wt = Math.ceil((gt * 0.004) / 0.05);
				if (gt) {
					const now = new Date(new Date().valueOf()).toLocaleTimeString();
					let finish = new Date(
						new Date().valueOf() + growTime
					).toLocaleTimeString();
					let pidG = ns.exec(
						growScript,
						host,
					...
					ns.print(
						`${finish} - grow ${target.hostname}: ${gt} in ${ns.nFormat(
							growTime / 1000,
							"0,000"
						)} seconds`
					);
