const showUsage = (ns) => {
  ...
  ns.tprint(
    '  buy <hostname> <gb> (<old hostname>) - purchase server (and replace old host)',
  );
  ns.tprint('');
};
...
export async function main(ns) {
  ...
  for (let i = 0; i < current.length; i++)
    current[i] = await ns.getServer(current[i]);
    ...
    case 'list':
      ...
      if (current.length === 0) {
        ...
        current.forEach((x, index) => {
          ...
          let freeRam = ns.nFormat(
            Math.floor(x.maxRam - x.ramUsed) * 1e9,
            '0,000.0b',
          );
      ...
      if (command === 'list') {
        for (let i = 0; i < options.length; i++) {
          if (
            options[i].cost < player.money &&
            options[i].cost > options[maxIndex].cost
          )
            maxIndex = i;
        }
      ...
      for (let i = min; i <= max; i++) {
        ...
        let gb = `${options[i].gb}`;
        ns.tprint(
          `${i === maxIndex ? 'INFO:' : '     '} ${gb.padStart(8)}GB for ${ns
            .nFormat(options[i].cost, '$0,000.0a')
            .padStart(7)} - ${buyCommand}`,
        );
      }
      break;
    case 'delete':
