...
export async function main(ns) {
  ...
  if (command === 'analyze' || command === 'details') {
    ns.tprint(
      `INFO: host ${host}, ram ${ns.nFormat(ram, '0,000')} gb, cores ${cores}`,
    );
  ...
  const findProcessing = (worker) => {
    let min = 0,
      max = batcher.processing.length - 1;
    while (min <= max) {
  ...
  const processIncomingMessages = () => {
    ...
    messages.forEach((msg) => {
      ...
      if (message === 'start') {
        ...
        if (
          batcher.compareWorkers({ id, eEnd }, batcher.processing[index]) === 0
        )
          EXIT('got start message for worker already in array!', { msg });

        // update worker with accurate info and insert into array at the right spot
        Object.assign(worker, { start, time, eEnd });
        batcher.processing = batcher.processing
          .slice(0, index)
          .concat([worker])
          .concat(batcher.processing.slice(index));
        ...
        if (
          worker.command !== 'weak' &&
          previous &&
          previous.command === worker.command
        ) {
        ...
        batcher.processing = batcher.processing
          .slice(0, oldIndex)
          .concat(batcher.processing.slice(oldIndex + 1));
        ...
        batcher.processing = batcher.processing
          .slice(0, newIndex)
          .concat([worker])
          .concat(batcher.processing.slice(newIndex));
        ...
        if (batcher.compareWorkers(worker, batcher.processing[index]) !== 0) {
          ...
          let err = {
            ...
            worker,
            index,
            processingLength: batcher.processing.length,
            previous: batcher.processing[index - 1],
            next: batcher.processing[index],
          };
          batcher.missing = batcher.missing || [];
          batcher.missing[batcher.missing.length] = err;
          ns.tprint(
            `ERROR: Got end message for ${worker.command} not in processing array!`,
          );
          ns.tprint(
            `INFO: ${batcher.processing[index - 1]?.id} -> ${id} <- ${
              batcher.processing[index]
            }`,
          );
          ...
          batcher.processing = batcher.processing
            .slice(0, index)
            .concat(batcher.processing.slice(index + 1));
        }
      } else {
  ...
  const createWorker = (host, command, threads, id, execTime, execEnd) => {
    ...
    try {
      ...
      EXIT(ERR, {
        ...
        worker,
        calculations: batcher.calculations,
      });
    }
    if (!worker.pid) {
      EXIT(`could not exec() script`, {
        args: [
          scriptFile,
          host,
          threads,
  ...
  ns.tprint(
    `    Expect results at ${new Date(
      new Date().valueOf() + ns.getWeakenTime(target),
    ).toLocaleTimeString()}`,
  );
  ...
  batcher.expectedWeak = Math.ceil(
    ns.getWeakenTime(target) / batcher.calculations.delay,
  ); // how many weakens will be running
  ...
  const removeOldProcessing = () => {
    ...
    while (
      batcher.processing.length &&
      i < batcher.processing.length &&
      batcher.processing[i].eEnd < time
    ) {
      i++;
    }
    if (i > 0) {
      obj.errors.push({
        message: `Removing ${i} processing that are 30 seconds late`,
        rows: batcher.processing.slice(0, i),
      });
    ...
    return (data) => {
      ...
      if (now - lastReport >= REPORT_DELAY) {
        ...
        if (batcher.firstHackFinish) {
          let activeSeconds =
            (new Date().valueOf() -
              batcher.firstHackFinish +
              batcher.calculations.delay) /
            1000;
          activePerHour = batcher.totalProfit / (activeSeconds / 3600);
        }
        let lastMinuteProfit =
          batcher.totalProfit - (batcher.lastTotalProfitReported || 0);
        ...
        const c1 = '\x1b[38;5;207m',
          c2 = '\x1b[38;5;75m',
          r = '\x1b[40m';
        ns.print(
          `Report: ${c1}${batcher.target}${r} (${batcher.host}): ` +
            `+${ns.nFormat(lastMinuteProfit, '$0,000.000a')}, ${ns.nFormat(
              batcher.totalProfit,
              '$0,000.000a',
            )} in ${ns.nFormat(seconds, '0,000')}s or ${c2}${ns.nFormat(
              perHour,
              '$0,000.000a',
            )}/h or ${c2}${ns.nFormat(activePerHour, '$0,000.000a')}/h`,
        );
  ...
  const updateCalculations = () => {
    ...
    if (hackTime < batcher.calculations.hTime) {
      batcher.server = ns.getServer(batcher.target);
      batcher.player = ns.getPlayer();
      let calculations = recalculateHGW(
        batcher.server,
        batcher.player,
        batcher.ram,
        batcher.cores,
      ...
      if (calculations) {
        ...
        batcher.hackTimeUpdates = batcher.hackTimeUpdates.slice(
          Math.max(batcher.hackTimeUpdates.length - 20, 0),
        );
        batcher.newLevel = batcher.player.skills.hacking;
        let lines = [];
        lines.push('INFO: Calculation Update:');
        if (batcher.calculations.hlvl !== calculations.hlvl || true)
          lines.push(
            `hlvl: ${batcher.calculations.hlvl} => ${calculations.hlvl}`,
          );
        if (batcher.calculations.ht !== calculations.ht)
          lines.push(`ht:${batcher.calculations.ht}=>${calculations.ht}`);
        if (batcher.calculations.gt !== calculations.gt)
        ...
        if (batcher.calculations.hm !== calculations.hm)
          lines.push(
            `h$:${ns.nFormat(batcher.calculations.hm, '$0.000a')}=>${ns.nFormat(
              calculations.hm,
              '$0.000a',
            )}`,
          );
        if (batcher.calculations.gm !== calculations.gm)
          lines.push(
            `g$:${ns.nFormat(batcher.calculations.gm, '$0.000a')}=>${ns.nFormat(
              calculations.gm,
              '$0.000a',
            )}`,
          );
        if (batcher.calculations.hTime !== calculations.hTime)
          lines.push(
            `hTime:${ns.nFormat(batcher.calculations.hTime, '0')}=>${ns.nFormat(
              calculations.hTime,
              '0',
            )}`,
          );
        ...
        // ns.print(lines.join(', '))
        ns.print(
          `INFO: Level ${calculations.hlvl}: hgw ${calculations.ht}/${calculations.gt}/${calculations.wt}, active ${batcher.counts.hack}/${batcher.counts.grow}/${batcher.counts.weak}`,
        );
      } else {
      ...
      if (server.moneyMax - server.moneyAvailable > batcher.calculations.gm) {
        for (let i = 0; i < batcher.processing.length; i++) {
          ...
          if (worker.command === 'hack') {
            ...
            batcher.processing = batcher.processing
              .slice(0, i)
              .concat(batcher.processing.slice(i + 1));
            return;
          }
        }
      }
  ...
  while (true) {
    ...
    if (batcher.nextWeak != 0 && new Date().valueOf() >= batcher.nextWeak) {
      ...
      if (
        batcher.createWorker(
          useHost,
          'weak',
          batcher.calculations.wt,
          id,
          duration,
          eEnd,
        )
    ...
    if (
      batcher.availableRam > missingRam + batcher.calculations.rG &&
      new Date().valueOf() - batcher.lastGrowCreatedAt >= 20
    ) {
      ...
      if (
        nextWorker &&
        nextWorker.eEnd < eEnd + 200 &&
        nextWorker.command === 'weak' &&
        nextWorker.eEnd >= eEnd + 20 &&
        nextWorker.eEnd <= eEnd + 50
      ) {
        if (
          batcher.createWorker(
            host,
            'grow',
            batcher.calculations.gt,
            id,
            duration,
            eEnd,
          )
    ...
    reportResults({
      availableRam: batcher.availableRam,
      hostMaxRam: batcher.hostMaxRam,
      expectedWeak: batcher.expectedWeak,
      possibleHacks: batcher.possibleHacks,
    });
    await ns.sleep(10);
  }
}
...
function recalculateHGW(server, player, ram, cores, calculations, ns) {
  ...
  let { hackThreads, growThreads } = solveForWeakens(
    wt,
    hackPercent,
    growPercentFn,
  );
  ...
  if (!gt) {
    ns.tprint(
      `ERROR: could not solve for hack and grow threads given ${wt} weaken threads on ${server.hostname}`,
    );
...
function calculateHGW(
  ...
  // let wtMax = wt ? wt : 100
  let wtMin = 1,
    wtMax = 1320; // 1320 will get you from 99 difficulty down to 33, the largest possible change
  let valid = null;
  while (wtMin <= wtMax) {
    ...
    let gtPossible = Math.trunc((wt * 0.05 - 0.002) / 0.008);
    ...
    let { hackThreads, growThreads } = solveForWeakens(
      ...
      growPercentFn,
      ns,
    ); // DEBUG change to solveForWeakensDbg
    let ht = hackThreads;
    let gt = growThreads;
    ...
    if (valid) {
      ...
      if ((result.wt - 1) * 0.5 > result.ht * 0.002 + result.gt * 0.004) {
        wtMax = wt - 1;
      } else {
        wtMin = wt + 1;
      }
...
function solveForWeakens(weakenThreads, hackPercent, growPercentFn, ns) {
  ...
  while (minH <= maxH) {
    ...
    if (G * 0.004 + midH * 0.002 > weakenThreads * 0.05) {
      maxH = midH - 1;
      continue;
    }
    validH = midH;
...
async function prepServer(ns, batcher) {
  ...
  while (
    ...
    let availableRam =
      hostS.maxRam - hostS.ramUsed - (host === 'home' ? 64 : 0);
    ...
    if (totalT > availableThreads) {
      ...
      let doneServer = {
        ...server,
        hackDifficulty: Math.max(
          server.hackDifficulty + gt * 0.004 - wt * 0.05,
        ...
        JSON.stringify(
          {
            ...
            wt,
            availableThreads,
            diff: server.hackDifficulty - server.minDifficulty,
    ...
    if (wt) {
      ...
      ns.print(
        `Weaking ${target} using ${wt} threads on ${host} - pid ${pids[0]}`,
      );
    }
    if (gt) {
      pids[1] = ns.exec(
        `/var/tmp/hgw-prep-grow.js`,
        host,
        gt,
        target,
        gt,
      ...
      ns.print(
        `Growing ${target} using ${gt} threads on ${host} - pid ${
          pids[1]
        } (need ${calcGt - gt} after)`,
      );
    }
    if (!(gt || wt)) {
      ns.print(
        `prepping doesn't need any threads?!?!  ${
          server.hackDifficulty - server.minDifficulty
        }, ${server.moneyMax - server.moneyAvailable}`,
      );
      break;
    ...
    while (
      ns.ps(host).filter((x) => x.pid === pids[0] || x.pid === pids[1]).length
    ) {
      await ns.sleep(1000);
    }
