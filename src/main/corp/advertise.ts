...
export async function main(ns: NS) {
  let divisionNames = ns.corporation.getCorporation().divisions;
  if (ns.args[0] && typeof ns.args[0] === 'string')
    divisionNames = ns.args[0].split(',');
  ...
  while (true) {
    ...
    for (let i = 0; i < info.divisions.length; i++) {
      let div = ns.corporation.getDivision(info.divisions[i]);
...
const fnum = (num: number) => {
  let e = Math.trunc(Math.log10(num) - 1);
  let ev = Math.pow(10, e);
  let rn = Math.trunc((num / ev) * 1000) / 1000;
  return `${rn}e+${e}`;
};
