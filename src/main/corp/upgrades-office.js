...
export async function main(ns) {
  let divisionNames = ns.corporation
    .getCorporation()
    .divisions.map((x) => x.name);
  ...
  const expandOffices = (divisionName, cityName, development = false) => {
    ...
    expandSize = Math.max(
      0,
      Math.min(expandSize, 5000 - office.employees.length),
    );
    if (expandSize > 0) {
      ns.tprint(
        `Expanding offices in ${divisionName} - ${cityName} by ${expandSize}`,
      );
  ...
  const hireEmployees = (divisionName, cityName) => {
    ...
    let counts = positions.reduce(
      (p, c) => Object.assign(p, { [c]: { count: 0, weight: 1 } }),
      {},
    );
    office = ns.corporation.getOffice(divisionName, cityName);
    let employees = office.employees.map((name) =>
      ns.corporation.getEmployee(divisionName, cityName, name),
    );
