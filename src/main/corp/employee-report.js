...
export async function main(ns) {
  ...
  for (let i = 0; i < corp.divisions.length; i++) {
    ...
    for (let j = 0; j < division.cities.length; j++) {
      ...
      let employees = office.employees.map((employeeName) =>
        ns.corporation.getEmployee(division.name, city, employeeName),
      );
      let totals = employees.reduce((acc, employee) => {
        attributes.forEach(
          (att) => (acc[att] = (acc[att] || 0) + employee[att]),
        );
        return acc;
      }, {});
      ...
      attributes.forEach(
        (att) =>
          (avg[att] = ((totals[att] || 0) / employees.length).toFixed(3)),
      );
      list.push(avg);
    }
  }
