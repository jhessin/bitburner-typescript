...
export async function main(ns) {
  ...
  if (!divisionName) {
    ns.tprint(
      'Division must be specified, one of: ' +
  ...
  if (!division) {
    ns.tprint(
      `WARNING: Division '${divisionName} not found, specify one of:\n  ` +
        corp.divisions.map((x) => x.name).join(', '),
    );
    return;
  }
  ...
  if (cityNames === 'all') {
    ...
    if (notFound) {
      ns.tprint(
        'WARNING: One or more cities not found, use CSV containing one or more:',
      );
  ...
  if (weights.length != 6) {
    ns.tprint(
      `WARNING: weights must be CSV of SIX numbers, i.e. 2,2,1,1,0.5,0)`,
    );
    return;
  }
  ...
  for (let i = 0; i < cities.length; i++) {
    ...
    let employeeCounts = weights.map((w) =>
      Math.trunc((w * employees.length) / totalWeights),
    );
...
function showUsage(ns) {
  ns.tprint(
    'INFO: run corp/reassign.js <division> [<city(s)|all>] [1,1,1,1,1,1]',
  );
  ns.tprint('INFO:  * division must be specified');
  ns.tprint('INFO:  * default is all cities, or enter CSV of city names');
  ns.tprint(
    'INFO:  * csv of weights for 6 jobs, default is 1 for all but 0 for training',
  );
}
