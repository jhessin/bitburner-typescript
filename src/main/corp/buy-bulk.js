      ...
      if (corp.hasWarehouse(divName, city) && useCity(city)) {
        ...
        weightNames.forEach((weightName, index) => {
          if (!div.products.find((x) => x === weightName)) {
            let targetStorage =
              wh.size * TARGET_STORAGE * (weights[index] / totalWeight);
            ...
            let purchaseAmountPerSecond =
              neededAmount >= targetAmount * 0.05
                ? Math.trunc(neededAmount / 10)
                : 0;
            purchaseAmountPerSecond = Math.trunc(purchaseAmountPerSecond);
            purchaseAmountPerSecond = Math.max(0, purchaseAmountPerSecond);
            purchaseAmountPerSecond = Math.min(
              2000000000,
              purchaseAmountPerSecond,
            );
            let args = [divName, city, mat.name, purchaseAmountPerSecond * 10];
            ns.print(
              `${weightName} want ${fnum(targetStorage, 0)} space or ${fnum(
                targetAmount,
                0,
              )} units, need ${fnum(neededAmount)}, buying ${fnum(
                purchaseAmountPerSecond,
                0,
              )} (${purchaseAmountPerSecond * 10}):`,
            ); // + JSON.stringify(args))
