        ...
        for (const stk of allStocks.sort(purchaseOrder)) {
          ...
          if (estEndOfCycleValue <= 2 * commission)
            log(
              ...
              (owned
                ? ''
                : `We currently have ${formatNumberShort(
                    stk.ownedShares(),
                    3,
                    1,
                  )} shares in ${stk.sym} valued at ${formatMoney(
                    stk.positionValue(),
                  )} ` +
                  `(${((100 * stk.positionValue()) / maxHoldings).toFixed(
                    1,
                  )}% of corpus, capped at ${(diversification * 100).toFixed(
                    1,
                  )}% by --diversification).\n`) +
                `Despite attractive ER of ${formatBP(stk.absReturn())}, ${
                  owned ? 'more ' : ''
                }${stk.sym} was not bought. ` +
                `\nBudget: ${formatMoney(
                  budget,
                )} can only buy ${numShares.toLocaleString('en')} ${
                  owned ? 'more ' : ''
                }shares @ ${formatMoney(purchasePrice)}. ` +
                `\nGiven an estimated ${
                  marketCycleLength - estTick
                } ticks left in market cycle, less ${stk
    ...
    if (prepSummary) {
      ...
      stk.debugLog =
        `${stk.sym.padEnd(5, ' ')} ${(stk.bullish() ? '+' : '-')
          ...
          .padEnd(3)} ` +
        `Prob:${(stk.prob * 100).toFixed(0).padStart(3)}% (t${probWindowLength
          .toFixed(0)
          .padStart(2)}:${(stk.longTermForecast * 100)
          .toFixed(0)
          .padStart(3)}%, ` +
...
async function doBuy(ns, stk, sharesToBuy) {
  ...
  log(
    ns,
    `INFO: ${long ? 'Buying  ' : 'Shorting'} ${formatNumberShort(
      sharesToBuy,
      3,
      3,
    ).padStart(5)} (` +
      `${
        stk.maxShares == sharesToBuy + stk.ownedShares()
          ? '@max shares'
          : `${formatNumberShort(
              sharesToBuy + stk.ownedShares(),
              3,
              3,
            ).padStart(5)}/${formatNumberShort(stk.maxShares, 3, 3).padStart(
              5,
            )}`
      }) ` +
      `${stk.sym.padEnd(5)} @ ${formatMoney(expectedPrice).padStart(
        9,
      )} for ${formatMoney(sharesToBuy * expectedPrice).padStart(9)} (Spread:${(
        stk.spread_pct * 100
      ).toFixed(2)}% ` +
      `ER:${formatBP(stk.expectedReturn()).padStart(8)}) Ticks to Profit: ${stk
        .timeToCoverTheSpread()
        .toFixed(2)}`,
    noisy,
    'info',
  ...
  if (price == 0) {
    ...
    if (playerMoney < sharesToBuy * expectedPrice)
      log(
        ns,
        `WARN: Failed to ${long ? 'buy' : 'short'} ${
          stk.sym
        } because money just recently dropped to ${formatMoney(
          playerMoney,
...
async function liquidate(ns) {
  ...
  log(
    ns,
    `Sold ${totalSharesLong.toLocaleString(
      'en',
    )} long shares and ${totalSharesShort.toLocaleString('en')} short shares ` +
    ...
    if (await tryBuy(ns, 'purchase4SMarketData'))
      log(
        ...
        `SUCCESS: Purchased 4SMarketData for ${formatMoney(cost4sData)} ` +
          `(At ${formatDuration(
            playerStats.playtimeSinceLastBitnode,
          )} into BitNode)`,
        true,
        'success',
      );
...
async function tryGetStockMarketAccess(ns, budget) {
  ...
  if (!hasWSE) {
    if (await tryBuy(ns, 'purchaseWseAccount'))
      log(
        ns,
        `SUCCESS: Purchased a WSE (stockmarket) account for ${formatMoney(
          costWseAccount,
        )} ` +
          `(At ${formatDuration(
            (await getPlayerInfo(ns)).playtimeSinceLastBitnode,
          )} into BitNode)`,
        true,
        'success',
      );
  ...
  if (await tryBuy(ns, 'purchaseTixApi')) {
    log(
      ns,
      `SUCCESS: Purchased Tix (stockmarket) Api access for ${formatMoney(
        costTixApi,
      )} ` +
        `(At ${formatDuration(
          (await getPlayerInfo(ns)).playtimeSinceLastBitnode,
