...
export async function main(ns) {
    ...
    try {
      if (
        ...
        !(await getNsDataThroughFile(
          ...
          '/Temp/travelToCity.txt',
          ['Aevum'],
        ))
      )
        return tailAndLog(
          ns,
          'ERROR: Sorry, you need at least 200k to travel to the casino.',
        );
    } catch (err) {
      return tailAndLog(
        ns,
        'ERROR: You must manually travel to to Aevum to use this script until you get SF4',
      );
    }
  }
  ...
  const btnSaveGame = await findRetry(
    ns,
  ...
  while (attempts++ <= 10) {
    ...
    try {
      ...
      if (btnStopAction) {
        ...
        log(
          ns,
          "It looks like we're on a focus screen. Stopping whatever we're doing...",
        );
      ...
      try {
        ...
        await click(
          await findRetry(ns, "//span[@aria-label = 'Iker Molina Casino']"),
        );
      } catch {
      ...
      const blackjack = await findRetry(
        ns,
      ...
      if (ns.ls('home', '/Temp/').length > 0) {
        ...
        if (await findRetry(ns, "//p[contains(text(), 'Count:')]", true, 10)) {
          ...
          return log(
            ns,
            'INFO: We appear to already have been previously kicked out of the casino.',
            true,
          );
        }
  ...
  while (true) {
    ...
    do {
      ...
      if (won === null) {
        ...
        const txtCount = await findRetry(
          ns,
        ...
        const shouldHit = options['use-basic-strategy']
          ? highCount < 17
          : shouldHitAdvanced(ns, txtCount);
        if (options['enable-logging'])
          log(
            ns,
            `INFO: Count is ${highCount}, we will ${
              shouldHit ? 'Hit' : 'Stay'
            }`,
          );
...
async function killAllOtherScripts(ns, removeRemoteFiles) {
  ...
  const allServers = await getNsDataThroughFile(
    ns,
    'scanAllServers(ns)',
    '/Temp/scanAllServers.txt',
  );
...
function getDealerCount() {
  const text = find("//p[contains(text(), 'Dealer')]/..").innerText.substring(
    8,
    9,
  );
