// from:
//    youtube: https://www.youtube.com/watch?v=5EOI-yY2tCU
//    github: https://github.com/chrisrabe/bitburner-automation
//    youtube (channel): https://www.youtube.com/channel/UCXVoS73T7gn9h4PHghPxpuw
//    youtube (github): https://www.youtube.com/watch?v=fbbaf-XpoUY&t=804s
...
export async function main(ns) {
  ...
  function getStonks() {
    ...
    for (const sym of stockSymbols) {
      ...
      const shortProfit =
        stock.shortPrice * (stock.shortPrice - stock.askPrice) - tradeFees;
      ...
      stock.summary = `${stock.sym}: ${stock.forecast.toFixed(
        3,
      )} +/- ${stock.volatility.toFixed(3)}`;
      stocks.push(stock);
    }
  ...
  function takeLongTendies(stock) {
    if (stock.forecast > 0.5) {
      ...
      ns.print(
        `INFO\t ${stock.summary} LONG ${ns.nFormat(curValue, '0a')} ${roi}%`,
      );
      ...
      stock.shares = 0;
      ns.print(
        `WARN\t${stock.summary} SOLD for ${ns.nFormat(
          saleProfit,
          '$0.0a',
        )} profit`,
      );
  ...
  function yolo(stocks) {
    ...
    for (const stock of stocks) {
      const money = ns.getPlayer().money - 6e10; // keep $60b on hand
      if (stock.forecast > 0.55) {
        if (money > riskThresh) {
          ...
          if (ns.stock.buyStock(stock.sym, sharesToBuy) > 0) {
            ns.print(
              `WARN\t${stock.summary}\t- LONG @ ${ns.nFormat(
                sharesToBuy,
                '$0.0a',
              )}`,
            );
