...
async function calcBatchParams(ns, target_name, hacking_levels = 1, t0 = 50) {
  ...
  const grow_base = ns.formulas.hacking.growPercent(
    target,
    1,
    player,
    host.cpuCores,
  ...
  for (let hack_threads = 1; hack_threads <= max_hack_threads; ++hack_threads) {
    ...
    const grow_threads = solveGrow(
      grow_base,
      target.moneyMax - money_taken,
      target.moneyMax,
    );
  ...
  const ram_max =
    host.maxRam - ns.getScriptRam('grader.js') - ns.getScriptRam('batch.js');
  const kW_max = Math.floor(1 + (weak_time_lo - 4 * t0) / (8 * t0));

  const params = {
    target: target_name,
    t0,
    max_hacking: player.skills.hacking,
    max_money: target.moneyMax,
    min_level: target.minDifficulty,
    dps: 0,
  };
...
async function runBatcher(ns, params, duration, minDuration) {
  ...
  for (let batch = 0; ; ++batch) {
    ...
    if (time_delay >= params.t0) {
      ns.print(
        ns.sprintf('WARN: %5d loop started %.2fms late', batch, time_delay),
      );
      dispatch = false;
    }
    ...
    if (
      (player.skills.hacking > params.max_hacking &&
        time_begin > time_end_min) ||
      time_begin > time_end
    ) {
      ns.print(ns.sprintf('ERROR: %5d Ending batching', batch));

      for (let i = 0; i < 4; ++i) {
        for (const pid of pids[i]) {
          ns.kill(pid, HOST);
        }
      }
      ...
      ns.print(
        ns.sprintf(
          'WARN: %5d Hacking increased to %d',
          batch,
    ...
    if (cur_level > params.min_level) {
      ns.print(
        ns.sprintf(
          'WARN: %5d Security level raised by %.3f',
          batch,
          cur_level - params.min_level,
        ),
      );

      // Kill hack and grow about to land
      ns.kill(pids[0][next_index[0]], HOST);
      pids[0][next_index[0]] = 0;
      ns.kill(pids[2][next_index[2]], HOST);
      pids[2][next_index[2]] = 0;
      ...
      ns.print(
        ns.sprintf(
          'WARN: %5d Money too low, at %.1f%%',
          batch,
          (100 * cur_money) / target.max_money,
        ),
      );
    ...
    if (dispatch) {
      ...
      if (dispatch_error) {
        ns.print(
          ns.sprintf('ERROR: %5d Could not dispatch %s', batch, scripts[i]),
        );
...
export async function main(ns) {
  ...
  while (true) {
    ...
    for (;;) {
      ...
      if (!params) {
        throw new Error(`Could not calculate batch params for ${target_name}`);
      }
      ...
      if (time_batch >= duration) {
        ns.tprintf(
          `Remaining time (${ns.nFormat(
            (duration - time_begin) / 1000,
            '0:00:00',
          )}) not enough to batch`,
        );
        break;
      ...
      ns.tprintf(
        `Beginning batch of ${params.target} at level ${
          params.hacking
        } at ${new Date(Date.now()).toLocaleTimeString()}`,
      );
      ns.tprintf(`         Money: \$${ns.nFormat(money_begin, '0.000a')}`);
      ns.tprintf(
        `    Start time: ${time_begin.toFixed(0).padStart(4)} (${ns.nFormat(
          time_begin,
          '0:00:00',
        )})`,
      );
      ns.tprintf(
        `    Batch time: ${time_batch.toFixed(0).padStart(4)} (${ns.nFormat(
          time_batch,
          '0:00:00',
        )})`,
      );
      ...
      ns.tprintf(
        `      End time: ${time_end.toFixed(0).padStart(4)} (${ns.nFormat(
          time_end,
          '0:00:00',
        )})`,
      );
      ...
      ns.tprintf(
        `        Profit: \$${ns.nFormat(money_end - money_begin, '0.000a')}`,
      );
