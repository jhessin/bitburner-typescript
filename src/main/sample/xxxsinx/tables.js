// from xxxsinx:
// https://github.com/xxxsinx/bitburner/blob/main/tables.js
  ...
  if (columns == undefined) {
    ...
    for (let i = 0; i < data[0].length; i++) {
      ...
      for (let ii = 0; ii < data.length; ii++) {
        ...
        let len =
          data[ii][i].text != undefined
            ? data[ii][i].text.length
            : data[ii][i].toString().length;
...
function PrintLine(ns, columns, data, style, printfunc = ns.print, highlight) {
  ...
  for (let c = 0; c < columns.length; c++) {
    if (data[c].style != undefined)
      printStack.push(
        { style: data[c].style },
        data[c].text.padEnd(columns[c].width),
      );
    else if (data[c].color != undefined)
      printStack.push(
        CreateStyle(data[c].color, highlight),
        data[c].text.padEnd(columns[c].width),
      );
    else
      printStack.push(
        CreateStyle('white', highlight),
        data[c].toString().padEnd(columns[c].width),
      );
    printStack.push('white', style[0][BAR]);
  }
...
export function ColorPrint(ns, stack, toTerminal = true) {
  ...
  for (let i = 0; i < stack.length; i += 2) {
    ...
    let match = COLORS.find(
      (s) => s.html == color || s.desc.toLowerCase() == color.toLowerCase(),
    );
    if (!match && color.startsWith('#'))
      match = FindHtmlColorEquivalent(ns, color);
    if (!match)
      match = COLORS.find((s) =>
        s.desc.toLowerCase().startsWith(color.toLowerCase()),
      );
...
function FindHtmlColorEquivalent(ns, htmlCode) {
  ...
  copy.sort(function (a, b) {
    ...
    let oa =
      Math.abs(ca.r - col.r) + Math.abs(ca.g - col.g) + Math.abs(ca.b - col.b);
    let ob =
      Math.abs(cb.r - col.r) + Math.abs(cb.g - col.g) + Math.abs(cb.b - col.b);
    return oa - ob;
  });
  ...
  for (var i = 0; i < formattedData.length; i++) {
    ...
    for (var j = 0; j < col.length; j++) {
      ...
      if (typeof formattedData[i][col[j]] == 'number') {
        ...
        if (number > 0)
          tabCell.innerHTML = addHtmlWithColor(
            'a',
            fmt.format(number),
            'green',
          );
