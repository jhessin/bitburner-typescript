// from xxxsinx:
// https://github.com/xxxsinx/bitburner/blob/main/stonks.js

import { PrintTable, DefaultStyle } from '/sample/xxxsinx/tables.js';
import { FormatMoney } from '/sample/xxxsinx/utils.js';
...
export async function main(ns) {
  ...
  if (!ns.stock.hasWSEAccount() || !ns.stock.hasTIXAPIAccess()) {
    ns.print(
      'ERROR: You need a Wse account and Tix Api access to run this script.',
    );
    ns.tprint(
      'ERROR: You need a Wse account and Tix Api access to run this script.',
    );
    ...
    if (TIME_TRACKING && performance.now() - started > BENCH_TIME) {
      ...
      ns.tprint(
        "FAIL: It's been an hour and the current balance is " +
          FormatMoney(ns, balance) +
          ' initial funds were ' +
          FormatMoney(ns, initialFunds) +
          ' profit: ' +
  ...
  for (const stonk of log) {
    ...
    ns.print(
      'WARN: Selling ' +
        ...
        stonk.sym +
        ' for ' +
        ns.nFormat(stonk.GetValue(), '$0.000a') +
        ' (' +
        ns.nFormat(stonk.GetProfit(), '$0.000a') +
        ' profit)',
    );
    if (dump)
      ns.tprint(
...
function BuyStonks(ns, log) {
  ...
  for (const stonk of stonks) {
    ...
    if (totalPrice > budget) {
      ns.print(
        'Budget is : ' +
          ns.nFormat(budget, '$0.000a') +
          ' and price is ' +
          ns.nFormat(totalPrice, '$0.000a'),
      );
      continue;
    }
    ...
    ns.print(
      'INFO: Buying ' +
        ...
        stonk.sym +
        ' at price ' +
        ns.nFormat(maxShares * sharePrice, '$0.000a'),
    );
    ...
    let spent =
      stonk.forecast < 0.5
        ? ns.stock.buyShort(stonk.sym, maxShares)
        : ns.stock.buyStock(stonk.sym, maxShares);
...
function ReportCurrentSnapshot(ns, stonks) {
  ...
  for (const stonk of stonks) {
    ...
    let forecast =
      stonk.forecast == 'N/A' ? stonk.forecast : stonk.forecast.toFixed(4);
    ...
    line.push({
      color: 'white',
      text: stonk.forecast >= 0.5 ? ' Long' : ' Short',
    });
    line.push({ color: 'white', text: ' ' + forecast });
    line.push(ForecastToGraph(stonk.forecast));
    line.push({
      color: 'white',
      text: ns.nFormat(stonk.nbShares + stonk.nbShorts, '0.0a').padStart(9),
    });
    line.push({
      color: 'white',
      text: ns.nFormat(stonk.GetValue(), '0.0a').padStart(9),
    });
    line.push({
      color: 'white',
      text: ns.nFormat(stonk.GetPricePaid(), '0.0a').padStart(9),
    });
    line.push({
      color: 'white',
      text: ns.nFormat(stonk.GetProfit(), '0.0a').padStart(9),
    });
  ...
  let totalWorth =
    total.paid + total.profit + ns.getServerMoneyAvailable('home');
...
export class Stonk {
  ...
  Update() {
    ...
    const [shares, avgPx, sharesShort, avgPxShort] = this.ns.stock.getPosition(
      this.sym,
    );
    ...
    if (g_tixMode) {
      ...
      if (this.snapshots.length == LOG_SIZE)
        this.forecast =
          nbUp /
          (LOG_SIZE -
            1); // -1 here because with 15 values, we only have 14 changes
  ...
  GetProfit() {
    ...
    let longProfit =
      this.nbShares * this.bidPrice - this.nbShares * this.avgPrice;
    let shortProfit =
      this.nbShorts * this.avgShortPrice - this.nbShorts * this.askPrice;
