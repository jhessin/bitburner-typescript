// from xxxsinx:
// https://github.com/xxxsinx/bitburner/blob/main/utils.js
...
export function ServerReport(
  ...
  printfunc('┌─────────────────────────────────────────────────────┐');
  printfunc(
    '│ ' + server.padStart(52 / 2 + server.length / 2).padEnd(52) + '│',
  );
  printfunc('├─────────────────────────────────────────────────────┤');
  printfunc(
    '│ ' +
      (
        'Money        : ' +
        ns.nFormat(so.moneyAvailable, '$0.000a') +
        ' / ' +
        ns.nFormat(so.moneyMax, '$0.000a') +
        ' (' +
