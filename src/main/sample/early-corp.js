...
export async function main(ns) {
  ...
  async function waitState(state, times = 1, onpoint = false) {
    for (let i = 0; i < times; i++) {
      while (CorpApi.getCorporation().state != state) {
        await ns.sleep(11);
      }
      if (onpoint) {
        while (CorpApi.getCorporation().state == state) {
  ...
  async function prep() {
    //divisions
    while (
      CorpApi.getCorporation(corp).divisions.length < division_goals[round]
    ) {
      let name = all_divisions[CorpApi.getCorporation(corp).divisions.length];
      CorpApi.expandIndustry(name, name);
    }
    //upgrades && unlocks
    while (CorpApi.getUpgradeLevel('Smart Storage') < smart_goals[round]) {
      await CorpApi.levelUpgrade('Smart Storage');
    }
    while (CorpApi.getUpgradeLevel('Project Insight') < project_goals[round]) {
      await CorpApi.levelUpgrade('Project Insight');
    }
    while (
      CorpApi.getUpgradeLevel('Neural Accelerators') < project_goals[round]
    ) {
      await CorpApi.levelUpgrade('Neural Accelerators');
    }
    while (
      CorpApi.getUpgradeLevel('Nuoptimal Nootropic Injector Implants') <
      project_goals[round]
    ) {
      await CorpApi.levelUpgrade('Nuoptimal Nootropic Injector Implants');
    }
    while (CorpApi.getUpgradeLevel('FocusWires') < project_goals[round]) {
      await CorpApi.levelUpgrade('FocusWires');
    }
    while (
      CorpApi.getUpgradeLevel('Speech Processor Implants') < speech_goals[round]
    ) {
      await CorpApi.levelUpgrade('Speech Processor Implants');
    }
    while (CorpApi.getUpgradeLevel('DreamSense') < dream_goals[round]) {
      await CorpApi.levelUpgrade('DreamSense');
    }
    while (CorpApi.getUpgradeLevel('ABC SalesBots') < abc_goals[round]) {
      await CorpApi.levelUpgrade('ABC SalesBots');
    }
    ...
    for (const division of CorpApi.getCorporation().divisions) {
      ...
      while (CorpApi.getDivision(division.name).cities.length < cities.length) {
        for (let city of cities) {
          if (!CorpApi.getDivision(division.name).cities.includes(city)) {
            await CorpApi.expandCity(division.name, city);
          }
        }
      }
      //buy some ads
      while (CorpApi.getHireAdVertCount(division.name) < adv_goals[round]) {
        await CorpApi.hireAdVert(division.name);
      }
      //buy Warehouses
      for (let city of cities) {
        if (CorpApi.hasWarehouse(division.name, city) == false) {
          await CorpApi.purchaseWarehouse(division.name, city);
        }
      }
      ...
      for (let city of cities) {
        ...
        while (
          CorpApi.getWarehouse(division.name, city).level < storage_goals[round]
        ) {
          await CorpApi.upgradeWarehouse(division.name, city);
          await ns.sleep(1);
        }
        //upgrade Office size to goal
        while (
          CorpApi.getOffice(division.name, city).size < employee_goals[round]
        ) {
          CorpApi.upgradeOfficeSize(division.name, city, 3);
        }
        //hire to max
        while (
          CorpApi.getOffice(division.name, city).employees.length <
          CorpApi.getOffice(division.name, city).size
        ) {
          await CorpApi.hireEmployee(division.name, city);
        }
    ...
    while (done < CorpApi.getCorporation().divisions.length) {
      ...
      for (const division of CorpApi.getCorporation().divisions) {
        ...
        for (let city of cities) {
          ...
          let tenergy = 0;
          let thappiness = 0;
          CorpApi.getOffice(division.name, city).employees.forEach(
            (x) => (tmorale += CorpApi.getEmployee(division.name, city, x).mor),
          );
          CorpApi.getOffice(division.name, city).employees.forEach(
            (x) => (tenergy += CorpApi.getEmployee(division.name, city, x).ene),
          );
          CorpApi.getOffice(division.name, city).employees.forEach(
            (x) =>
              (thappiness += CorpApi.getEmployee(division.name, city, x).hap),
          );
          tmorale =
            tmorale / CorpApi.getOffice(division.name, city).employees.length;
          tenergy =
            tenergy / CorpApi.getOffice(division.name, city).employees.length;
          thappiness =
            thappiness /
            CorpApi.getOffice(division.name, city).employees.length;
          let party = 3e6 * (round + 1);
          tmorale > 99.8 && thappiness > 99.8 ? (party = 1e5) : null;
          tmorale < 100 || thappiness < 100
            ? CorpApi.throwParty(division.name, city, party)
            : null;
  ...
  async function takeOffer() {
    ...
    for (const division of CorpApi.getCorporation().divisions) {
      for (let city of cities) {
        ...
        const amt =
          CorpApi.getWarehouse(division.name, city).size -
          CorpApi.getWarehouse(division.name, city).sizeUsed -
          5;
    ...
    while (
      CorpApi.getWarehouse(main_division, cities[0]).sizeUsed <
      CorpApi.getWarehouse(main_division, cities[0]).size - 5
    ) {
      await ns.sleep(100);
    }
    //reset buys to 0
    for (const division of CorpApi.getCorporation().divisions) {
      for (let city of cities) {
        CorpApi.buyMaterial(division.name, city, 'AI Cores', 0);
      }
    }
  ...
  async function end() {
    !CorpApi.hasUnlockUpgrade('Smart Supply') &&
    CorpApi.getUnlockUpgradeCost('Smart Supply') <
      CorpApi.getCorporation().funds
    ...
    for (const division of CorpApi.getCorporation().divisions) {
      for (let city of cities) {
        ...
        await CorpApi.setAutoJobAssignment(
          ...
          city,
          'Research & Development',
          CorpApi.getOffice(division.name, city).employees.length / 3,
        );
        await CorpApi.setAutoJobAssignment(
          division.name,
          city,
  ...
  async function getCorp() {
    ...
    if (!player.hasCorporation) {
      if (player.bitNodeN == 3) {
        ...
        while (ns.getPlayer().money < 15e10) {
          ns.clearLog();
  ...
  function analyze(offer) {
    ...
    round == 1
      ? ns.write(analyzefile, '\n' + result, 'a')
      : ns.write(analyzefile, ' ' + result, 'a');
  }
}
