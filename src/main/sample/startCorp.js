...
export async function main(ns) {
  ...
  */
    ...
    for (let city of cities) {
      ...
      for (let i = 0; i < 3; i++) {
        await ns.corporation.assignJob(
          agricultureName,
          city,
          ns.corporation.hireEmployee(agricultureName, city).name,
          jobs[i],
        );
      }
  ...
  async function initialPurchases() {
    ...
    while (
      ns.corporation.getMaterial(agricultureName, cities[0], boostMaterials[0])
        .qty == 0
    ) {
  ...
  async function waitForTheLazyFucksToGetTheirShitTogether() {
    while (true) {
      let avgs = [0, 0, 0];
      for (let city of cities) {
        for (let emp of ns.corporation.getOffice(agricultureName, city)
          .employees) {
          avgs[0] +=
            ns.corporation.getEmployee(agricultureName, city, emp).mor / 18;
          avgs[1] +=
            ns.corporation.getEmployee(agricultureName, city, emp).hap / 18;
          avgs[2] +=
            ns.corporation.getEmployee(agricultureName, city, emp).ene / 18;
        }
      }
  ...
  async function invest(i) {
    ...
    ns.tprint(
      'investment offer: ' +
        ns.nFormat(ns.corporation.getInvestmentOffer().funds, '0.00a'),
    );
  ...
  async function expandToTobacco() {
    ...
    try {
      await ns.corporation.assignJob(
        tobaccoName,
        cities[0],
        ns.corporation.hireEmployee(tobaccoName, cities[0]).name,
        jobs[0],
      );
      await ns.corporation.assignJob(
        tobaccoName,
    ...
    try {
      for (let city of cities) {
        ...
        await ns.corporation.assignJob(
          tobaccoName,
          city,
          ns.corporation.hireEmployee(tobaccoName, city).name,
          jobs[0],
        );
        await ns.corporation.assignJob(
          tobaccoName,
    ...
    ns.corporation.makeProduct(
      ...
      cities[0],
      'Stick 1',
      ns.corporation.getCorporation().funds / 20,
      ns.corporation.getCorporation().funds / 20,
    );
    try {
      for (let i = 2; i < 6; i++) {
    ...
    while (
      ...
      ns.corporation.getHireAdVertCost(tobaccoName)
    ) {
