...
export async function main(ns) {
  ...
  let upgradeGearNames = gearNames.filter((x) =>
    ['Weapon', 'Armor', 'Vehicle', 'Rootkit'].find(
      (y) => ns.gang.getEquipmentType(x) === y,
    ),
  );
  // ns.tprint('upgradeGearNames: ' + JSON.stringify(upgradeGearNames))
  let totalCost = upgradeGearNames.reduce(
    (p, c) => p + ns.gang.getEquipmentCost(c),
    0,
  );
  ns.tprint(
    'TOTAL COST: ' +
  ...
  while (true) {
    ...
    if (p.money > totalCost + 1000000) {
      ...
      let names = ns.gang
        .getMemberNames()
        .filter((x) => getMaxAscensionResult(x) >= (x === 'Boots' ? 1.1 : 1.2));
      for (let i = 0; i < names.length; i++) {
        ...
        ns.tprint(
          `Ascending ${name}: ` +
            JSON.stringify({
              maxa: maxa.toFixed(3),
              ...Object.fromEntries(
                Object.entries(asc).map((x) => {
                  x[1] = Math.trunc(x[1] * 1000) / 1000;
                  return x;
                }),
              ),
            }),
        ...
        upgradeGearNames.forEach((gearName) =>
          ns.gang.purchaseEquipment(name, gearName),
        );
