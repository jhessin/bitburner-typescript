...
export async function main(ns) {
  ...
  while (true) {
    ...
    let needRepAugs = augs.filter(
      (x) => x.needed && x.affordable && !x.haveRep,
    );
    ...
    if (neededRep > 0) {
      ...
      if (corpInfo && corpInfo.funds > 0) {
        ...
        if (possibleRep > neededRep) {
          ns.tprint(
            `Bribing '${FACTION} with $${fnum(neededRep * 1e9)} to get ${fnum(
              neededRep,
            )} rep`,
          );
    ...
    const possible = augs.filter(
      (x) => x.needed && x.affordable && x.haveRep && x.havePrereqs,
    );
    ...
    while (possible.length > 0) {
      ...
      if (next.cost <= augs.playerMoney) {
        ns.tprint(
          `WARNING: Purchasing aug: ${next.name} for $${fnum(next.cost)}`,
        );
  ...
  while (
    augs.playerMoney >=
    (augs.find((x) => x.name === 'NeuroFlux Governor')?.cost || 0)
  ) {
    ...
    if (neededRep > 0) {
      ...
      ns.tprint(
        `INFO: Bribing '${FACTION} with $${fnum(neededRep * 1e9)} to get ${fnum(
          neededRep,
        )} rep`,
      );
      ns.corporation.bribe(FACTION, neededRep * 1e9);
