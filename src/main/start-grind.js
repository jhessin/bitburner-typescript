...
export async function main(ns) {
	...
	if (!monitor) {
		// TODO: Fix this, probably just not getting the value right above
		let grinderHost = ns.getHostname();
		let killPids = ns
			.ps(grinderHost)
			.filter((x) => x.filename.indexOf("test-grind") >= 0)
			.map((x) => x.pid);
		...
		if (killPids.length)
			ns.tprint(
				`INFO: grind - killed ${killPids.length} scripts on ${grinderHost}`
			);
	...
	while (true) {
		...
		for (let i = 0; i < targets.length; i++) {
			...
			try {
				let income =
					ns.getScriptIncome("/remote/hack-loop.js", host, target, 0) || 0;
				let expGain =
					ns.getScriptExpGain("/remote/hack-loop.js", host, target, 0) || 0;
		...
		let table = createTable(
			results.map((x) => ({
				...
				expGain:
					x.expGain <= 0 ? "waiting" : ns.nFormat(x.expGain || 0, "0,000"),
			})),
