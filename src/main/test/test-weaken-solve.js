...
export async function main(ns2) {
  ...
  for (let wt = 1; wt <= 100; wt++) {
    ...
    if (wt && hackThreads && growThreads) {
      ...
      let profit =
        Math.trunc(3600000 / batchDelay) *
        hm *
        hacking.hackChance(prepped, player);
      ...
      up.skills = {
        ...player.skills,
        hacking: player.skills.hacking + ADD_LEVELS,
      };
  ...
  ns.tprint(
    `results @lvl ${player.skills.hacking}:\n` + rows.join('\n') + '\n',
  );

  let rows2 = createTable(
    results2.map((x) => {
      try {
        return {
          wt: x.wt,
  ...
  ns.tprint(
    `results @lvl ${player.skills.hacking + ADD_LEVELS}:\n` +
      rows2.join('\n') +
      '\n',
  );
}
