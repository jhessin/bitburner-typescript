...
export async function main(ns) {
  ...
  let weakenThreads = Math.ceil(
    (maxThreads * 0.002 + growThreads * 0.004) / 0.05,
  );
  ns.print(
    JSON.stringify({
      hp,
      ...
      growThreads,
      weakenThreads,
      maxDiff: maxThreads * 0.002,
    }),
  );
  let hackTime = hacking.hackTime(server, player);
  let hacked = {
    ...server,
    hackDifficulty: server.baseDifficulty + maxThreads * 0.002 + 1,
  };
  ...
  let killPids = ns
    .ps(host)
    .filter((x) => x.filename.indexOf('-loop') >= 0)
    .map((x) => x.pid);
  ...
  if (killPids.length)
    ns.tprint(`INFO: grind - killed ${killPids.length} scripts on ${host}`);
  ...
  ns.print(
    `Starting hacks...  First complete in ${ns.nFormat(
      hackTime / 1000,
      '0,000.0',
    )} sec at ${finish.toLocaleTimeString()}`,
  );
  ...
  let hackThreads =
    (ns.getServerMaxRam(host) - ns.getServerUsedRam(host) - reserveGb) / 1.7;
  if (hackThreads > 0)
  ...
  while (true) {
    ...
    ns.print(
      `${new Date().toLocaleTimeString()}: ${ns.nFormat(
        xpGain || 0,
        '0.000a',
      )}`,
    );
