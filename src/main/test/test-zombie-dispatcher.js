...
export async function main(ns) {
  ...
  let runOnZombie = (zombie, duration, command, ...args) => {
    ns.print(
      `runOnZombie command: ${command} taking ${ns.nFormat(
        duration || 0,
        '0',
      )} ms, args: ${JSON.stringify(args)}`,
    );
  ...
  let findZombie = async () => {
    ...
    while (true) {
      for (let i = 0; i < zombies.length; i++) {
        if ((zombies[i].whenAvaialable || 0) + 2000 < new Date().valueOf()) {
          // double fudge-factor
  ...
  let runOnZombie2 = (zombie, duration, command, ...args) => {
    ns.print(
      `runOnZombie2 command: ${command} taking ${ns.nFormat(
        duration || 0,
        '0',
      )} ms, args: ${JSON.stringify(args)}`,
    );
  ...
  while (true) {
    ...
    ns.tprint(
      `Called hack using zombie, waiting ${
        ns.nFormat(hTime / 1000, '0.0') + 's'
      }`,
    );
    ...
    ns.tprint(
      `Called weaken using zombie, waiting ${
        ns.nFormat(wTime / 1000, '0.0') + 's'
      }`,
    );
    ...
    ns.tprint(
      `Called grow using zombie, waiting ${
        ns.nFormat(gTime / 1000, '0.0') + 's'
      }`,
    );
