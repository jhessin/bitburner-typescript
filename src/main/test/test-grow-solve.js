...
export async function main(ns) {
  ...
  for (let ht = 10; ht < 259; ht += ht < 250 ? 10 : 1) {
    ...
    let hacked = {
      ...baseServer,
      moneyAvailable: remaining,
      hackDifficulty: baseServer.hackDifficulty + 0.002 * ht,
    };
    ...
    let naiveMoney =
      remaining *
      (hacking.growPercent(hacked, naiveThreads, player, cores) - 1);
    ...
    let calcThreads = hacking.numCycleForGrowth(
      hacked,
      1 + calcPercentNeeded,
      player,
      cores,
    ...
    let calcMoney =
      remaining * (hacking.growPercent(hacked, calcThreads, player, cores) - 1);
    ...
    let newMoney =
      remaining * (hacking.growPercent(hacked, newThreads, player, cores) - 1);
    let newMinus = newThreads - 1;
    let newMinus$ =
      remaining * (hacking.growPercent(hacked, newMinus, player, cores) - 1);
    let newPlus = newThreads + 1;
    let newPlus$ =
      remaining * (hacking.growPercent(hacked, newPlus, player, cores) - 1);

    let sgThreads = solveGrow(gp1 + 1, remaining, moneyMax);
    let sg$ =
      remaining * (hacking.growPercent(hacked, sgThreads, player, cores) - 1);

    let nsgThreads = newSolveGrow(gp1 + 1, remaining, moneyMax);
    let nsg$ =
      remaining * (hacking.growPercent(hacked, nsgThreads, player, cores) - 1);
