...
function buy(ns, stock, numShares) {
  ...
  ns.print(
    `${stock.sym}: Can buy ${ns.nFormat(numShares, '0,000')} for ${ns.nFormat(
      cost,
      '$0,000.000a',
    )}`,
  );
  ...
  if (price) {
    ns.print(
      `Bought ${stock.sym} for ${ns.nFormat(numShares * price, '$0.00a')}`,
    );
  } else {
    ns.print(
      `ERROR: Could not buy ${ns.nFormat(numShares, '0,000')} of ${
        stock.sym
      } at ${ns.nFormat(ask, '$0.00a')}`,
    );
  }
  return numShares;
}
...
export async function main(ns) {
  ...
  for (let i = 0; i < symbols.length; i++) stocks.push({ sym: symbols[i] });
