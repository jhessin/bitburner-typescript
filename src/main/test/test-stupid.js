...
export async function main(ns) {
  ...
  let batchRam =
    ht * 1.7 + gt * growsPerHack * 1.75 + wt * weakensPerHack * 1.75;
  ...
  for (let id = 0; id < growCount; id++)
    schedule.push({
      id,
      script: scripts.grow,
      threads: gt,
      offset: growStart + id * growDelay,
    });
  for (let id = 0; id < hackCount; id++)
    schedule.push({
      id,
      script: scripts.hack,
      threads: ht,
      offset: hackStart + id * hackDelay,
    });
  ...
  ns.print(
    `Schedule set, starting...  Complete in ${ns.nFormat(
      (hTime * 4) / 1000,
      '0,000.0',
    )} sec at ${finish.toLocaleTimeString()}`,
  );
...
function createWorkerScripts(ns) {
  ...
  ns.write(
    scripts.grow,
    `export async function main(ns) { let [target, id] = ns.args; while (true) await ns.grow(target) }`,
    'w',
  );

  ns.write(
    scripts.weak,
    `export async function main(ns) { let [target, id] = ns.args; while (true) await ns.weaken(target) }`,
    'w',
  );
