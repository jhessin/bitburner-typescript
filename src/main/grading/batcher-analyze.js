...
export function main(ns) {
  ...
  if (!host || !target) {
    ns.tprint('ERROR: must pass host and target');
    return;
  }
  ...
  const calculate = (hackThreads) => {
    ...
    const weakenThreads = Math.max(
      Math.ceil(growThreadsEach / 12.5),
      Math.ceil(hackThreads / 25),
    );
    const totalActiveThreads =
      hackThreads + growThreads * 8 + weakenThreads * 5 * 3;
    ...
    const ramPerBatch =
      hackThreads * 1.7 +
      growThreadsEach * 2 * 4 * 1.75 +
      weakenThreads * 3 * 5 * 1.75;
    ...
    let calcRamUsage =
      calcWeakens * weakenThreads * 1.75 +
      calcGrows * growThreadsEach * 1.75 +
      calcHacks * hackThreads * 1.7;
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    if (!values) break;
    if (
      byWeakenThreads.length !== 0 &&
      byWeakenThreads[byWeakenThreads.length - 1].weakenThreads ===
        values.weakenThreads
    ) {
