...
export async function main(ns) {
  ...
  let ram =
    command === 'analyze'
      ? hostServer.maxRam
      : hostServer.maxRam - hostServer.ramUsed; // use all ram when anlyzing only
  let cores = hostServer.cpuCores;

  const calculations =
    target === 'all'
      ? analyzeAllServers(ns, ram, cores)
      : [analyzeServer(ns, ram, target, cores)];
  ...
  for (let t = use.startWeakens; t < use.weakenTime; t += use.delayWeakens) {
    commands.push({ t, script: 'weaken', finish: t + use.weakenTime });
  ...
  for (let i = 0; i < use.hackTime / use.delayHacks; i++) {
    let t =
      Math.trunc(weakenFinishes[i * 2] - use.delayWeakens / 2) - use.hackTime;
    commands.push({ t, script: 'hack', finish: t + use.hackTime });
  }

  // hacks will take place (delayWeakens/2) before the second and every other weaken
  for (let i = 0; i < use.growTime / use.delayGrows; i++) {
    let t =
      Math.trunc(weakenFinishes[i * 2 + 1] - use.delayWeakens / 2) -
      use.growTime;
    commands.push({ t, script: 'grow', finish: t + use.growTime });
  }
  ...
  let actualCounts = commands.reduce(
    (p, c) => {
      p[c.script]++;
      return p;
    },
  ...
  let countsTable = createTable(
    countsRows.concat([totals]).map((x) => {
  ...
  let readyTime = new Date(
    new Date().valueOf() + use.weakenTime,
  ).toLocaleTimeString();
  ns.print(
    `Scheduling ${commands.length} scripts over ${ns.nFormat(
      use.weakenTime / 1000,
      '0.0',
    )} seconds (active ${readyTime})`,
  );
  ...
  let LOG_DELAY = 60000;
  let next =
    Math.ceil((activeStart - start) / 60000) * 60000 + new Date().valueOf();
  let logs = [];
  while (true) {
    let time = new Date().valueOf();
    let ms = next - time;
    if (ms > 0) await ns.sleep(ms);
    next += LOG_DELAY; // WHY ISN'T THIS WAITING 60 SECONDS?S?S?
    time = new Date().valueOf();
    let income =
      ns.getScriptIncome(ns.getScriptName(), ns.getHostname(), ...ns.args) || 0;
    // income is per minute I think, but only works when scripts finish I think :(
    ...
    let table = createTable(
      logs.map((x) => ({
        target: target,
        time: x.totalTime ? ns.nFormat(x.totalTime / 1000, '00:00:00') : 'ERR',
        income: x.income ? ns.nFormat(x.income, '$0.00a') : 'ERR',
        active: x.activeTime
          ? ns.nFormat(x.activeTime / 1000, '00:00:00')
          : 'ERR',
...
async function createScriptsAndCopyToHost(ns, hostname) {
  ...
  export async function main(ns) {
    ...
    while (true) {
      ...
      ns.clearLog()
      ns.print('Result: ' + ns.nFormat((result || 1) - 1, '0.000%'))
    }
  }`;
  ...
  export async function main(ns) {
    ...
    while (true) {
      ...
      ns.clearLog()
      ns.print('Result: ' + ns.nFormat(result || 0, '0.000'))
    }
  }`;
...
function report(ns, list, useLog = false) {
  ...
  let results = sorted
    .filter((x) => x.profitPerHour)
    .map((x) => ({
      hostname: x.hostname,
      profit: x.profitPerHour
        ? ns.nFormat(x.profitPerHour, '$0,000.0a')
        : 'ERR',
      ...
      hack$: x.successfulHackMoney
        ? ns.nFormat(x.successfulHackMoney, '$0.0a')
        : 'ERR',
      'hack%': x.hackPercent ? ns.nFormat(x.hackPercent, '0.00%') : 'ERR',
      'grow%': x.growPercent ? ns.nFormat(x.growPercent, '0.00%') : 'ERR',
      '#': x.hackScriptsPerHackTime
        ? ns.nFormat(x.hackScriptsPerHackTime, '0')
        : 'ERR',
...
function analyzeAllServers(ns, ram, cores) {
  ...
  let results = list
    .map((x) => analyzeServer(ns, ram, x.hostname, cores))
    .filter((x) => x);
...
function calculateForServer(ns, server, player, ram, cores = 1) {
  ...
  for (let i = 1; i < 1000; i++) {
    ...
    let calculated = calculateForWeakenThreads(
      ns,
      server,
      player,
      ram,
  ...
  ns.tprint(
    `could not find valid with delayWeakens >= 80 for ${server.hostname}`,
  );
  return last;
}
 ...
 */
function calculateForWeakenThreads(
  ns,
  server,
  player,
  ram,
  ...
  if (weakenThreads * 25 < hackThreads) {
    // too many hack threads?!?  upside-down server growth
    // use gp = hp/(1-hp)
  ...
  if (
    weakenThreads * 25 < hackThreads ||
    Math.trunc(weakenThreads * 12.5) > growThreads
  ) {
  ...
  let weakenScriptsPerWeakenTime =
    (hackScriptsPerHackTime * 5 + (growScriptsPerGrowTime * 5) / 4) *
    extraWeakenFactor;

  let successfulHackMoney = hackPercent * server.moneyMax;
  let expectedHackMoney = successfulHackMoney * hackChance;
  let profitPerHour =
    (expectedHackMoney * hackScriptsPerHackTime * 3600000) / hackTime;
  let expectedExpForHacksPerHackScript =
    hackExp * (hackChance + (1 - hackChance) / 4) * hackThreads;
  let expectedExpForGrowsPerGrowScript = hackExp * growThreads;
  let expectedExpForWeakensPerWeakenScript = hackExp * weakenThreads;
  let expPerHour =
    (expectedExpForHacksPerHackScript * hackScriptsPerHackTime * 3600000) /
      hackTime +
    (expectedExpForGrowsPerGrowScript * growScriptsPerGrowTime * 3600000) /
      growTime +
    (expectedExpForWeakensPerWeakenScript *
      weakenScriptsPerWeakenTime *
      3600000) /
      weakenTime;
  ...
  let calculatedRam =
    Math.trunc(hackTime / delayHacks) * 1.7 * hackThreads +
