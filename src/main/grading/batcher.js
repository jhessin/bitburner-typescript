...
export async function main(ns) {
  ...
  const copyFilesToServer = async (hostname) => {
    ...
    await ns.scp(
      ['/remote/weak.js', '/remote/grow.js', '/remote/hack.js'],
      hostname,
    );
  };
  if (host !== 'home') {
  ...
  const processIncomingMessages = () => {
    ...
    messages.forEach((msg) => {
      ...
      if (!worker)
        EXIT(`Got message for unknown worker ${id}`, { msg, workers });
      ...
      if (message === 'start') {
        ...
        if (compareWorkers({ id, eEnd }, processing[index]) === 0)
        ...
        processing = processing
          .slice(0, oldIndex)
          .concat(processing.slice(oldIndex + 1));
        ...
        processing = processing
          .slice(0, newIndex)
          .concat([worker])
          .concat(processing.slice(newIndex));
        executions[command]++;
      } else if (message === 'end') {
        // ------------------------------ end message ------------------------------
        ...
        if (compareWorkers(worker, processing[index]) !== 0)
          EXIT(`got end message for worker missing from array!`, {
            msg,
            worker,
            index,
        ...
        processing = processing
          .slice(0, index)
          .concat(processing.slice(index + 1));
        delete workers[id];
        counts[command]--;
        executions[command]++;
      } else {
  ...
  const createWorker = (host, command, threads, id, execTime, execEnd) => {
    ...
    if (!worker.pid) {
      EXIT(`could not exec() script`, {
        args: [scriptFile, host, threads, target, id, command, PORT],
        worker,
      });
    }
    return worker;
  ...
  const createReporter = () => {
    ...
    return (data) => {
      ...
      if (now - lastReport >= 10000) {
        ...
        ns.tprint(
          `${ns.nFormat(totalProfit, '$0,000.0a')} in ${ns.nFormat(
            seconds,
            '0,000',
          )} or ${ns.nFormat(perHour, '$0,000.0a')} per hour`,
        );
      }
    };
  };
  ...
  while (true) {
    ...
    if (ram < hackScriptRam)
      EXIT('Not enough ram!  Tweak your settings...', {
        ram,
        hostMaxRam,
        hackScriptRam,
      });
    ...
    let missingRam =
      missingHackRam + missingWeakRam + RAM_SAFETY_FACTOR * hackScriptRam;
    if (ram > missingRam + growScriptRam) {
      ...
      const previousTwoAreWeak =
        processing[index - 1]?.command === 'weak' &&
        processing[index - 2]?.command === 'weak';
      const nextTwoAreWeak =
        processing[index]?.command === 'weak' &&
        processing[index + 1]?.command === 'weak';
    ...
    if (ram >= hackScriptRam) {
      ...
      if (firstHack) {
        ...
        if (
          processing.length > 0 &&
          processing[0].eEnd - eEnd < 200 &&
          processing[0].eEnd > eEnd
        ) {
        ...
        for (let i = index; i < processing.length; i++) {
          let w = processing[i];
          if (w.command === 'hack') break;
          if (w.command === 'grow' && Math.abs(w.eEnd - eEnd) > 15)
            futureGrows++;
        }
        ...
        const previousIsWeak =
          processing[index - 1]?.command === 'weak' &&
          processing[index - 1 - skipPrevious]?.command === 'weak';
        const nextIsWeak =
          processing[index]?.command === 'weak' &&
          processing[index + skipCurrent]?.command === 'weak';
