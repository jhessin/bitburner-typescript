		...
		while (new Date().valueOf() < restartAt) {
			...
			ns.print(
				`Restart in ${ns.nFormat(
					...
					"0.0"
				)} min at ${new Date(restartAt).toLocaleTimeString()}`
			);
			ns.print(table.join("\n"));
...
function killAllByName(ns, command) {
	...
	scanServer("home");
	runnable = Object.values(servers)
		.filter((x) => x.hasAdminRights && x.maxRam > 0 && x.hostname !== "home")
		.concat([{ hostname: "home" }]);
	...
	runnable.forEach((x) => {
		if (
			...
			list.forEach((info) => {
				if (
					info.filename.indexOf(command) >= 0 ||
					JSON.stringify(info.args).indexOf(command) >= 0
				) {
					count++;
