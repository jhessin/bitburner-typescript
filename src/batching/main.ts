...
export async function main(ns: NS) {
  ...
  function getRunnableServers(): string[] {
    ...
    return getServers(ns, (s) => {
      ...
      ns.scp([hackScript, growScript, weakenScript, hgwScript, basicHack], s);
      return true;
    });
  }
  ...
  let targets: string[] = getServers(ns).filter((s) => {
  ...
  targets.sort((a, b) => {
    function value(server: string) {
      return (
        ns.getServerMaxMoney(server) / ns.getServerMinSecurityLevel(server)
      );
    }
