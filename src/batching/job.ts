...
export class Job {
	...
	get threads() {
		...
		const hackThreads = Math.floor(this.hackPercent / ns.formulas.hacking.hackPercent(server, player)) || 1;
		server.moneyAvailable = this.hackPercent * ns.getServerMaxMoney(target);
		const growThreads = ns.formulas.hacking.growThreads(server, player, ns.getServerMaxMoney(target)) || 1;
	...
	get ram() {
		const growRam =
			this.ns.getScriptRam(growScript) * this.threads;
		const weakenRam =
			this.ns.getScriptRam(weakenScript) * this.threads;
		switch (this.jobType) {
			case JobType.Grow:
				return growRam;
			case JobType.Hack:
