...
export class Batch {
	...
	execute(hosts: string[]) {
		...
		const host2 = hosts[1];
		const host3 = hosts[2];
		const host1freeRam = this.ns.getServerMaxRam(host1) - this.ns.getServerUsedRam(host1)
		switch (this.mode) {
			case BatchMode.HGW:
				switch (hosts.length) {
					case 1:
			...
			case BatchMode.HWGW:
				switch (hosts.length) {
					...
					case 3:
						...
						const host2freeRam = this.ns.getServerMaxRam(host2) - this.ns.getServerUsedRam(host2)
						weaken.execute(host1);
						// how about 2?
						if (host1freeRam > weaken.ram * 2) {
							weaken.execute(host1, spacer * 2)
