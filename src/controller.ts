...
const TYPES: (keyof typeof COSTS)[] = ["hack", "weaken1", "grow", "weaken2"];
...
class Job {
	...
	constructor(type: keyof typeof COSTS, metrics: Metrics, server = "none") {
...
export class Metrics {
	...
	calculate(ns: NS, greed = this.greed) {
		...
		const gThreads = Math.ceil(
			ns.growthAnalyze(server, maxMoney / (maxMoney - maxMoney * tGreed)),
		);
...
export class RamNet {
	...
	#blocks: { server: string; ram: number }[] = []; // A list of every server and how much ram it has available.
	...
	constructor(ns: NS, servers: string[]) {
		for (const server of servers) {
			if (ns.hasRootAccess(server)) {
				...
				if (ram >= 1.6) {
					// Make sure there's enough ram on the server to run at least one script.
	...
	*/
	...
	finish(job: { server: string; cost: number }) {
 ...
 * @param {RamNet} ramNet
 */
export function optimizeBatch(ns: NS, metrics: Metrics, ramNet: RamNet) {
	...
	while (greed > minGreed) {
		...
		const amount = maxMoney * greed;
		const hThreads = Math.max(
			Math.floor(ns.hackAnalyzeThreads(metrics.target, amount)),
			1,
		);
		const tGreed = hPercent * hThreads;
		const gThreads = Math.ceil(
...
export async function main(ns: NS) {
	...
	*/
	...
	for (const type of TYPES) {
		...
		metrics.ends[type] =
			Date.now() + metrics.wTime + metrics.spacer * OFFSETS[type] + 100;
	...
	const timer = setInterval(() => {
		ns.ui.clearTerminal();
		ns.tprint(
			`Hacking $${ns.formatNumber(metrics.maxMoney * metrics.greed)} from ${
				metrics.target
			}`,
		);
		ns.tprint(
			`Running batch: ETA ${ns.tFormat(metrics.ends.weaken2 - Date.now())}`,
		);
	}, 1000);
